...
2. Query the right project first:
   ...
   - Shopify / 点石 / 傲飞 / order-name lifecycle logs: query `k8s-log-c19589a718db24dbb835525e4e8f2c2a0`.
...
3. Use narrow searches before broad scans:
   - When field names or types are uncertain, call `sls_inspect_index` first. Prefer indexed field queries for known fields and use raw full-text search only when the field is unknown.
   ...
   - For a trace that might cross DSers and 点石, use `trace_timeline` before manually joining rows. Its duration is the observed log timestamp interval, not a guaranteed tracing span duration.
...
Prefer running a small Python SLS SDK snippet from the current workspace when existing helpers are available, because it avoids shell quoting problems. Load `.env` explicitly with `load_dotenv('/Users/baiyuxin/Documents/查查查/.env')`.
...
For SQL/SPL aggregation, include `LIMIT` and `ORDER BY` in the query; do not rely on raw-query `offset` or `reverse` controls.
